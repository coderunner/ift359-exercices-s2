...
export type Action = null // À Faire

// Table qui encode quelle action bat quelle autre action
...
// Fonctions de constructions des différents types d'action
...
// Type qui représente une stratégie qui, à partir du contexte de la partie, retourne l'action du joueur
...
// À faire
// Déterminer les action des deux joueurs
// Déterminer le résultat
// Ajuster le pointage
// Retourner le nouveau round à ajouter à l'ihstorique de la partie
}
*/
...
export function jouerPartie(j1: Joueur, j2: Joueur, pointageCible: number): PartieJouee {
  function jouer(roundsJoues: Round[], pointageCourrant: Pointage): PartieJouee {
    // À faire
    // Écrire cette fonction récursive qui 'joue' des rounds de RPC jusqu'à ce qu'un joueur atteingne le poiuntageCible.
  }
...
function afficherCommentaire(nom: string, action: Action, resultat: "gagnant" | "perdant" | "nul") {
    // À faire
    // Selon l'action (switch)
    // si roche et que le joueur est gagnant, afficher commentaireSiGagnant si présent
    // si papier et que le joueur est perdant, afficher commentaireSiPerdant si présent
    // si ciseaux, afficher commentaire si présent
}
*/
